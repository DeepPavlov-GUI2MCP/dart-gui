...
def stats_from_records(records: Sequence[dict[str, Any]], *, shard_count: int) -> PretokenizedStats:
    rollout_dirs = {
...
def stats_from_manifest(manifest: Mapping[str, Any], *, shard_count: int) -> PretokenizedStats:
    expanded_rows = manifest.get("expanded_rows")
    if not isinstance(expanded_rows, int):
        raise ValueError("manifest.json must contain integer expanded_rows for fast validation")
    rollouts = manifest.get("rollouts")
    tasks = manifest.get("tasks")
    goals: int | None = None
    variants_per_task = manifest.get("variants_per_task")
    if isinstance(variants_per_task, int):
        goals = variants_per_task
    return PretokenizedStats(
        actual_rows=expanded_rows,
        rollouts=rollouts if isinstance(rollouts, int) else 0,
...
def manifest_supports_fast_validation(manifest: Mapping[str, Any]) -> bool:
    return isinstance(manifest.get("expanded_rows"), int)


def spot_check_shards(shard_paths: Sequence[Path]) -> None:
    if not shard_paths:
        raise ValueError("No pretokenized shards found for spot check")
    for shard_path in (shard_paths[0], shard_paths[-1]):
...
def collect_pretokenized_stats(pretokenized_dir: Path) -> PretokenizedStats:
    ...
    for shard_path in shard_paths:
        records.extend(load_shard_records(shard_path))
    return stats_from_records(records, shard_count=len(shard_paths))


def collect_pretokenized_stats_fast(
    pretokenized_dir: Path,
    ...
    if not shard_paths:
        raise ValueError(f"No pretokenized shards found under {pretokenized_dir}")
    expected_shard_count = manifest.get("shard_count")
    if isinstance(expected_shard_count, int) and expected_shard_count != len(shard_paths):
        raise ValueError(
            f"pretokenized shard count mismatch: found {len(shard_paths)}, "
            f"manifest expects {expected_shard_count}"
        )
    spot_check_shards(shard_paths)
    return stats_from_manifest(manifest, shard_count=len(shard_paths))
